...
function SnapTile({ icon, label, value, color }: { icon: string; label: string; value: string | number; color?: string }) {
  ...
  return (
    <div style={{
      background: 'var(--surface-alt)', border: '1px solid rgba(255,158,66,0.1)',
      borderRadius: '6px', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    }}>
      <div>
        <p style={{ color: 'var(--text-muted)', fontSize: '11px', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>{label}</p>
        <p style={{ color: '#ffffff', fontWeight: 800, fontSize: '26px', lineHeight: 1, fontFamily: 'Outfit, sans-serif' }}>{value}</p>
      </div>
...
function ToggleRow({ label, sub, checked }: { label: string; sub?: string; checked?: boolean }) {
  ...
  return (
    <div style={{
      ...
      border: '1px solid rgba(255,158,66,0.08)', borderRadius: '6px',
    }}>
      <div>
      ...
      <button type="button" onClick={() => setOn(!on)} style={{
        ...
        <span style={{ position: 'absolute', top: '2px', left: on ? '20px' : '2px', width: '16px', height: '16px', borderRadius: '50%', background: on ? '#0a0a0a' : '#ffffff', transition: 'left 0.2s' }} />
      </button>
    </div>
  );
}
    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh' }}>
      ...
      <main className="page-container animate-slide-up" style={{ padding: '24px 0', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: '20px', borderBottom: '1px solid rgba(255,158,66,0.15)' }}>
          <h1 style={{ color: '#ffffff', fontWeight: 700, fontSize: '24px', margin: 0, fontFamily: 'Outfit, sans-serif' }}>My Profile</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '16px' }}>
          ...
          <div className="card" style={{ padding: '28px 32px', display: 'flex', alignItems: 'center', gap: '24px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: '-40px', right: '-40px', width: '180px', height: '180px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,158,66,0.10) 0%, transparent 70%)', pointerEvents: 'none' }} />
            <div style={{
              width: '72px', height: '72px', borderRadius: '50%',
              background: 'linear-gradient(135deg, var(--accent), var(--accent-soft))',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '24px', fontWeight: 800, color: '#0a0a0a', flexShrink: 0,
              border: '3px solid rgba(255,158,66,0.3)', fontFamily: 'Outfit, sans-serif'
            }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '13px', margin: '0 0 8px' }}>{email}</p>
          ...
          <div className="card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
            <div style={{
              ...
              background: `rgba(${readinessScore >= 70 ? '80,200,120' : readinessScore >= 40 ? '255,158,66' : '239,68,68'}, 0.06)`
            }}>
              <span style={{ fontSize: '32px', fontWeight: 800, color: '#ffffff', fontFamily: 'Outfit, sans-serif', lineHeight: 1 }}>{readinessScore}%</span>
              <span style={{ fontSize: '9px', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Ready</span>
            </div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '12px', textAlign: 'center', margin: 0 }}>Career Readiness Score</p>
            <Link href="/dashboard/skills" style={{ color: 'var(--accent)', fontSize: '12px', fontWeight: 600 }}>Improve Score →</Link>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
          ...
          <SnapTile icon="◉" label="Career Matches" value={careerRecs.length} color="#ff9e42" />
          ...
          <Section>
            ...
            <div style={{ background: 'var(--surface-alt)', border: '1px solid var(--border-dark)', borderRadius: '6px', padding: '16px 18px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <span style={{ color: 'var(--text-muted)', fontSize: '11px' }}>Last changed: N/A</span>
              </div>
        ...
        <Section>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginTop: '-10px' }}>Set your target career path so we can personalise recommendations and skill gap analysis.</p>
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center', marginTop: '8px', flexWrap: 'wrap' }}>
            <Link href="/dashboard/skills" style={{ color: 'var(--accent)', fontSize: '13px', textDecoration: 'none', fontWeight: 600 }}>View Skill Gap Analysis →</Link>
            <Link href="/dashboard/courses" style={{ color: 'var(--text-secondary)', fontSize: '13px', textDecoration: 'none' }}>Find Courses</Link>
            <Link href="/dashboard/roadmaps" style={{ color: 'var(--text-secondary)', fontSize: '13px', textDecoration: 'none' }}>Learning Roadmaps</Link>
          </div>
        </Section>
        ...
        {skillRecs.length > 0 && (
          <Section>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginTop: '-10px' }}>AI-analyzed skills to boost your career readiness for {targetCareer}.</p>
              ...
              {skillRecs.map((rec: any, idx: number) => (
                <div key={rec.id || idx} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      <p style={{ color: '#ffffff', fontWeight: 600, fontSize: '14px', margin: 0 }}>{rec.skill_name || rec.skill?.name || 'Skill'}</p>
                      {rec.reason && <p style={{ color: 'var(--text-secondary)', fontSize: '12px', margin: '2px 0 0' }}>{rec.reason}</p>}
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span className="badge badge-muted" style={{ fontSize: '11px' }}>{rec.recommended_level || 'Beginner'}</span>
                    <span style={{ color: 'var(--text-muted)', fontSize: '11px' }}>Priority #{rec.priority_order || idx + 1}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {careerRecs.length > 0 && (
          <Section>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginTop: '-10px' }}>AI-recommended careers based on your profile and skills.</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {careerRecs.map((rec: any, idx: number) => (
                <div key={idx} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <div style={{
                      width: '40px', height: '40px', borderRadius: '50%', border: '2px solid var(--accent)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                  ...
                  <Link href="/dashboard/career" style={{ color: 'var(--accent)', fontSize: '13px', fontWeight: 600 }}>Explore →</Link>
                </div>
              ))}
            </div>
          </Section>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', paddingBottom: '20px', flexWrap: 'wrap' }}>
          ...
          <Link href="/dashboard" style={{ color: 'var(--text-secondary)', fontSize: '13px', textDecoration: 'none', fontWeight: 600 }}>Back to Dashboard</Link>
        </div>
      </main>
    </div>
  );
